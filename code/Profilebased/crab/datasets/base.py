...
def load_reviews():
    ...
    #Read data
    data_m = np.loadtxt(base_dir + 'dataset_review.csv',
                delimiter=',', dtype=str)
    item_ids = []
    user_ids = []
    data_review = {}
    for user_id, item_id, rating in data_m:
        ...
        u_ix = user_ids.index(user_id) + 1
        i_ix = item_ids.index(item_id) + 1
        data_review.setdefault(u_ix, {})
        #print u_ix,i_ix,rating
        data_review[u_ix][i_ix] = float(rating)
    ...
    fdescr = open(dirname(__file__) + '/descr/review.rst')

    return Bunch(data=data_review, item_ids=data_titles,
                 user_ids=data_users, DESCR=fdescr.read())
...
def load_sample_reviews():
    ...
    #Read data
    data_m = np.loadtxt(base_dir + 'dataset_review_PARTIAL.csv',
                delimiter=',', dtype=str)
    item_ids = []
    user_ids = []
    data_review = {}
    ...
    fdescr = open(dirname(__file__) + '/descr/reviews.rst')
